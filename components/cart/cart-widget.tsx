  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-3 right-3 z-50 inline-flex min-h-12 items-center gap-2 rounded-full border border-[#d8b994]/35 bg-ink px-3 py-2 text-xs font-semibold text-[#fffaf4] shadow-[0_18px_44px_rgba(42,27,18,0.28)] transition hover:bg-[#6d4a2d] sm:bottom-6 sm:right-6 sm:min-h-16 sm:gap-3 sm:px-6 sm:py-3 sm:text-base"
      >
        <ShoppingBag className="h-5 w-5 sm:h-7 sm:w-7" />
        <span className="hidden min-[420px]:inline">Order Cart</span>
        <span className="grid h-7 min-w-7 place-items-center rounded-full bg-[#fffaf4] px-2 text-xs text-ink sm:h-8 sm:min-w-8 sm:text-sm">{count}</span>
      </button>
          ...
          <aside className="absolute right-0 top-0 flex h-full w-full max-w-md flex-col bg-[#fffaf4] shadow-soft">
                      ...
                      <div>
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            ...
                            {item.categoryName ? <p className="mt-1 text-xs text-ink/45">{toTitleCase(item.categoryName)}</p> : null}
                          </div>
            ...
            <footer className="border-t border-ink/10 p-5">
              <a
                ...
                className={`inline-flex w-full items-center justify-center gap-2 rounded-full bg-ink px-5 py-4 text-sm font-medium text-[#fffaf4] ${
